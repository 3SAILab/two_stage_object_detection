...
class HarDNetFeatureExtraction(nn.Module):
    def __init__(self, depth_wise=True, arch=39):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        max_pool = True
        ...
        if arch==85:
            #HarDNet85
            first_ch  = [48, 96]
            ch_list = [  192, 256, 320, 480, 720, 1024]
        ...
        for i in range(blks):
            ...
            if downSamp[i] == 1:
                if max_pool:
                    ...
                    self.base.append ( DWConvLayer(ch, stride=1) )
        
        self.base.append(nn.Conv2d(ch_list[-1], ch_list[-1], 3, 2, 1, groups=ch_list[-1]))
        self.base.append(nn.ReLU())
        self.base.append(nn.Conv2d(ch_list[-1], ch_list[-1], 3, 2, 1, groups=ch_list[-1]))
        self.base.append(nn.Conv2d(ch_list[-1], 512, 1, groups=512))
...
class HarNetClassifier(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        return self.clssifier(x)

if __name__ == "__main__":
    model = HarDNetFeatureExtraction(depth_wise=True, arch=39).to("cuda")
    classify = HarNetClassifier()
    # print(model)
    x = torch.randn(1, 3, 600, 600).to("cuda")
    y = model(x).to("cuda")
    print(y.shape)
